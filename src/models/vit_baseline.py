"""
vit_baseline.py — Vision Transformer (ViT) baseline classifier.

Uses timm to load a pretrained ViT-Base/16 and replaces the head with a
single-logit output for binary hallucination-risk classification of Hebrew
PDF page images.
"""
...
class ViTClassifier(nn.Module):
    """
    ViT backbone with a single-logit output for binary classification.
    ...
    extractor (CLS-token pooled), and a linear head is attached on top.
    """
    ...
    def __init__(
        ...
        # num_classes=0 returns a feature extractor (pooled CLS embedding)
